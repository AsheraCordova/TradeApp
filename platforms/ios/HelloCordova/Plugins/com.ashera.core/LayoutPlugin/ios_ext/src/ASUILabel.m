...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (!self) return nil;
    //self->drawableTintMode = kCGBlendModeSourceIn;
    return self;
}
...
{
    ASBaseMeasurableView *measurableView = ((ASBaseMeasurableView*)[_widget asWidget]);
    ASRectM* bounds = [measurableView getWidgetBoundsWithInt:rect.size.width withInt:rect.size.height withNSObjectArray:nil];
    CGRect rectPad = CGRectMake(bounds->x_, bounds->y_, bounds->width_, bounds->height_);
    [super drawTextInRect: rectPad];
}
