...
export abstract class ScrollViewImpl<T> extends ViewGroupImpl<T>{
	...
	@Type(() => CommandAttr)
	@Expose({ name: "iosPreventAutoScroll" })
	iosPreventAutoScroll!:CommandAttr<boolean>| undefined;
...
this.foregroundGravity.setTransformer('gravity');		return this.thisPointer;
	...
	public setIosPreventAutoScroll(value : boolean) : T {
		this.resetIfRequired();
		if (this.iosPreventAutoScroll == null || this.iosPreventAutoScroll == undefined) {
			this.iosPreventAutoScroll = new CommandAttr<boolean>();
		}
		
		this.iosPreventAutoScroll.setSetter(true);
		this.iosPreventAutoScroll.setValue(value);
		this.orderSet++;
		this.iosPreventAutoScroll.setOrderSet(this.orderSet);
		return this.thisPointer;
	}
