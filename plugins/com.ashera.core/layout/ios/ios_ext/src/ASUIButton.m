...
- (void)mylayoutSubviews {
    if (self.titleLabel.numberOfLines == 0) {
        ASBaseMeasurableView* mview = [_widget asWidget];
        int paddingTop = [mview getCompoundPaddingTop];
        int  paddingAdjust = self.frame.size.height - (self.titleLabel.frame.size.height + paddingTop);
        if (paddingAdjust < 0) {
            self.titleEdgeInsets = UIEdgeInsetsMake(paddingTop - paddingAdjust, self.titleEdgeInsets.left, self.titleEdgeInsets.bottom, self.titleEdgeInsets.right);
        }
    }
}
