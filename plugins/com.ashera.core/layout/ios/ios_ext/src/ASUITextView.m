...
@interface ASUITextView () {
@public
	//start - props
	//end - props
}
@end
@implementation ASUITextView
@synthesize topCorrect;
//start - body
...
- (BOOL)isTopCorrectNeeded {
    return self.topCorrect != 0 && [self contentSize].height <= [self bounds].size.height;
}

- (void)setContentOffset:(CGPoint)contentOffset {
    if( [self isTopCorrectNeeded]) {
        return [super setContentOffset:CGPointMake(contentOffset.x, self.topCorrect)];
    }
